...
def get_json (mylist):
    global n
    if mylist['start_timestamp'] > 1634832000 and mylist['start_timestamp'] < 1634918400:
    #if mylist['start_timestamp'] > 1635091200 and mylist['start_timestamp'] < 1635177600:
    #if True:
        hijacker=mylist['suspicious_as']
        victim=mylist['before_as']
        prefix_len=mylist['prefix'][-2:]
        prefix_list.append(int(prefix_len))
        ...
        # if duration>=3 and mylist['level']=='high':
        #     print(duration)
        #     high_list.append(victim)
        ...
        return
        # date0=mylist['start_datetime'][0:10]
        # index=date_dict[date0]
        # start=mylist['start_datetime']
        # end=mylist['end_datetime']
        ...
        if mylist['prefix'] not in prefix_set:
            victim=mylist['before_as_country']
            victim_list.append(victim)
            prefix_set.add(mylist['prefix'])
        ...
        rate_affected=len_all_vp+':'+len_affected_as
        affected_rate.append(rate_affected)


def draw_number ():

    plt.plot(date_l, number_list,'b',label='total attacks')
    plt.plot(date_l,number_high,'g',label='attacks of high level')
    plt.xlabel("date")
    plt.ylabel("total attacks")
    plt.xlim(0,4)
    plt.legend()
    plt.title("web attack of case 1")
    plt.show()
...
def draw_cdf():
    ...
    plt.show()
    plt.savefig('case1_duration_cdf_1.png')


def prefix_length_cdf():
    count1 = Counter(prefix_list)
    ...
    plt.show()
    plt.savefig('case1_prefix_length.png')

    print('prefix length:')
    print(prefix_dict)


file=open('case1.jsonl')
